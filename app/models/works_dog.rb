...
#
  ...
  scope :ord_complete_stage, joins(:stages_dog).order("stages_dog.date_stop")
  ...
  def self.contracts(type)
    ...
    # 3 - все
    if type == 1 || type == 2
      works = self.jselect_works.where("works_dog.status < 3").ord_complete_stage.jgroup_works
    end
    if type == 3
      works = self.jselect_works.ord_complete_stage.jgroup_works
    end
    works.map!{|x| x.work_id}
    if type == 2
      works_completed = self.jselect_works.where("works_dog.status = 3").ord_complete_stage.jgroup_works
      works_completed.map!{|x| x.work_id}
