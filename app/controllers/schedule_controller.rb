class ScheduleController < ApplicationController
  ...
  def stages
    @fio = session[:my_name]
    contracts_list = WorksDog.contracts(cookies[:sort_stages])
    stageless_list = WorksDog.stageless_contracts(cookies[:sort_stages])
    ...
    if list.empty?
      ...
      @stages = WorksDog.stages_list(cookies[:sort_stages])
      @tasks = WorksDog.stageless_list(cookies[:sort_stages])
    end
  end 
